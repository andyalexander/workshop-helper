...
> **This section states the bench-verified model** (`R_centreline`, measured to the vertex of
> the two centrelines). The original outside-edge/`R_outside` reasoning was **falsified at the
> bench** — see [#17](https://github.com/andyalexander/workshop-helper/issues/17) and
> [#22](https://github.com/andyalexander/workshop-helper/issues/22) — and is preserved in the
> dated amendment blocks further down, not discarded.
...
> to the **vertex of the two centrelines**. `setback = R_centreline × tan(θ/2)`.
...
```
setback   = R_centreline × tan(θ/2)     ← the headline Output, to the vertex of the two centrelines
...
the wrong number. Reference the **projected centreline** — the vertex of the two centrelines —
because the owner marks back to the mid-line (*"forget about 'the corner'"*, [#17](https://github.com/andyalexander/workshop-helper/issues/17),
which falsified the earlier outside-edge reading at the bench). Every Output's label must name
...
### Calibrating at 90° — store `R_centreline`, never a bare "setback"

**The key implementation trap.** At 90°, `tan(45°) = 1`, so:

```
setback(90°) ≡ R_centreline     exactly
```
